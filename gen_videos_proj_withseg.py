...
def gen_interp_video(G, mp4: str, ws, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False,move2cpu=False ,device=torch.device('cuda'), **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    ...
    # video_out = imageio.get_writer(mp4, mode='I', duration=1000//60, codec='libx264', **video_kwargs)
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                ...
                if gen_shapes and frame_idx == 0:
                    ...
                    if move2cpu:
                        sigmas = torch.zeros((samples.shape[0], samples.shape[1], 1),dtype=torch.float16,device=device)
                    else:
                    ...
                    with tqdm(total = samples.shape[1]) as pbar:
                        with torch.no_grad():
                            while head < samples.shape[1]:
                                ...
                                if move2cpu:
                                    sigmas[:, head:head+max_batch] = sigma.to(torch.float16)
                                else:
                    ...
                    if output_ply:
                        ...
                        print(outdirs,mp4.replace('.mp4', '.ply'))
...
def generate_images(
    ...
    cfg: str='FFHQ',
    image_mode: str='image',
    sampling_multiplier: float=1,
    ...
    gen_interp_video(G=G, mp4=output, ws=ws, bitrate='100M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames, psi=truncation_psi, truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, gen_shapes=shapes,move2cpu=move2cpu, device=device)
    if move2cpu:
        # G.cpu()
        # del(ws)
        del G
